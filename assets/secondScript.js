...
$(document).ready(function() {
    ...
    fetch(
            `https://salty-mountain-68764.herokuapp.com/https://tasty.p.rapidapi.com/recipes/list?from=0&size=100&tags=date_night&q=${ingredient}`, {
                method: "GET",
                headers: {
        ...
        .then((data) => {
            ...
            for (
                var index = 0; index < data.results[i].sections[0].components.length; index++
            ) {
                var ingredientItem = document.createElement("li");

                ingredientItem.textContent =
                    data.results[i].sections[0].components[index].raw_text;

                ingredientList.appendChild(ingredientItem);
            }

            for (
                var index = 0; index < data.results[i].instructions.length; index++
            ) {
                var instructionItem = document.createElement("li");

                instructionItem.textContent =
                    data.results[i].instructions[index].display_text;
    ...
    fetch(
            `https://salty-mountain-68764.herokuapp.com/https://api.themoviedb.org/3/discover/movie?api_key=1f0c12a5a877dc629a002fa2c6169442&language=en-US&sort_by=popularity.desc&include_adult=false&page=10&with_genres=${genre}`, {}
        )
        .then((response) => {
            return response.json();
        })
...
function randomRecipeFetch() {
    ...
    fetch(
            `https://salty-mountain-68764.herokuapp.com/https://tasty.p.rapidapi.com/recipes/list?from=0&size=100&tags=date_night`, {
                method: "GET",
                headers: {
                    "x-rapidapi-key": "9db6727369mshcadc7a2e617b437p14279djsnff586e532c9e",
                    "x-rapidapi-host": "tasty.p.rapidapi.com",
...
function randomMovieFetch() {
    ...
    fetch(
            `https://salty-mountain-68764.herokuapp.com/https://api.themoviedb.org/3/discover/movie?api_key=1f0c12a5a877dc629a002fa2c6169442&language=en-US&sort_by=popularity.desc&include_adult=false&page=10`, {}
        )
        .then((response) => {
            return response.json();
        })
